...
static GstStaticPadTemplate sink_templ = GST_STATIC_PAD_TEMPLATE ("sink",
    ...
    GST_STATIC_CAPS ("application/x-subtitle; application/x-subtitle-sami; "
        ...
        "application/x-subtitle-lrc; application/x-subtitle-ebutt; ANY")
    );
#endif
...
handle_buffer (GstTtmlParse * self, GstBuffer * buf)
{
  ...
  if (g_strcmp0 (self->subtitle_codec, "EBUTT") == 0) {
    ...
    for (subtitle = subtitle_list; subtitle; subtitle = subtitle->next) {
      ...
      GstClockTime last_subtitle_end_time = GST_BUFFER_PTS (op_buffer) + GST_BUFFER_DURATION (op_buffer);

      if (last_subtitle_end_time >= self->segment.position) {
    ...
    for (; subtitle; subtitle = subtitle->next) {
      gst_buffer_unref (subtitle->data);
    }
